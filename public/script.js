...
// ==============================
// COLETAR TODOS OS DADOS DA TELA
// ==============================
...
// ==============================
// FUNÇÃO PRINCIPAL: ENVIAR AO BACKEND
// ==============================

async function extrairAnalise(salvarNoHistorico = false) {
  // Atualiza os dados da tela (caso exista)
  if (typeof atualizarMonitorSolo === "function") {
    await atualizarMonitorSolo();
  }

  // Pega todos os dados
  const dadosParaSalvar = await coletarDados();
  console.log("Dados capturados:", dadosParaSalvar);

  // Salvar no banco só se o botão pediu
  if (salvarNoHistorico) {
    try {
      ...
      if (!res.ok) {
        ...
        showNotification("Erro ao salvar análise: " + erro, 'error', 5000);
